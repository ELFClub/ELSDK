 ...
 *  @brief 切换系统语言
 *
 *  @param languageType 语言类型，ELSDKDefines中有声明
 */
- (void)setLanguage:(ELLanguageType)languageType;
 ...
 *
 */
- (void)login:(ELCallbackBlock)block;

- (void)login:(Boolean)isCanClose andBlock:(ELCallbackBlock)block;

/*
 * @brief 用户登录失效的回调，包括注销，用户中心，cookie失效等情况的回调
 */
- (void)logoutCallback:(ELCallbackBlock)Block;

/**
 *  @brief 用户点击退出，不会跳转到登录页面
 *
 */
- (void)logout:(ELCallbackBlock)Block;
 ...
 */
- (void)userInfo:(ELCallbackBlock)block;
 ...
 *  @param  block 回调block
 */
- (void)showGameManager:(NSString *)title block:(ELCallbackBlock)block;
 ...
 *
 */
- (void)isBindEmail:(ELCallbackBlock)block;

@end

#pragma mark - 内购 **************************************************

@interface ELSDK(ELIAP)
...
- (void)buyWithProductId:(NSString *)productId
           ...
           callbackBlock:(ELCallbackBlock)block ;
